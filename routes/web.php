...
Route::group(['domain' => domain()], function () {
    ...
    Route::name('backend.')
        ...
        Route::name('tags.')->prefix('tags')->group(function () {
            ...
            Route::get('{tag}')->name('edit')->uses('TagsController@form')->where('tag', '[a-z0-9-]+');
            Route::put('{tag}')->name('update')->uses('TagsController@update')->where('tag', '[a-z0-9-]+');
            Route::get('{tag}/logs')->name('logs')->uses('TagsController@logs')->where('tag', '[a-z0-9-]+');
            Route::delete('{tag}')->name('delete')->uses('TagsController@delete')->where('tag', '[a-z0-9-]+');
        });
